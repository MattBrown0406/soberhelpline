...
import { Calendar, ChevronRight, FileCheck, Printer } from "lucide-react";
...
export default function BoundaryClarityWorksheet() {
  ...
  const handleReset = () => {
    ...
    setEnforcementPlan([]); setSelfReflection("");
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={values} onChange={(e) => setValues(e.target.value)} placeholder="Write your top 5 values..." rows={3} />
          </div>
          <div>
            <Label className="font-medium">2. Which value feels most violated right now?</Label>
            <Textarea value={mostViolated} onChange={(e) => setMostViolated(e.target.value)} placeholder="Describe which value..." rows={2} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={problemBehavior} onChange={(e) => setProblemBehavior(e.target.value)} placeholder="Describe the specific behavior..." rows={2} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={boundaryStatement} onChange={(e) => setBoundaryStatement(e.target.value)} placeholder="If _________ happens, I will _________." rows={2} />
          </div>
          <div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={violation1} onChange={(e) => setViolation1(e.target.value)} rows={2} />
          </div>
          <div>
            <Label className="font-medium">2nd violation response:</Label>
            <Textarea value={violation2} onChange={(e) => setViolation2(e.target.value)} rows={2} />
          </div>
          <div>
            <Label className="font-medium">Final consequence:</Label>
            <Textarea value={finalConsequence} onChange={(e) => setFinalConsequence(e.target.value)} rows={2} />
          </div>
          <div>
            <Label className="font-medium mb-2 block">Am I actually willing to follow through?</Label>
            <RadioGroup value={willingFollowThrough} onValueChange={setWillingFollowThrough} className="space-y-2">
              {["Yes", "No", "Unsure"].map((opt) => (
                <div key={opt} className="flex items-center space-x-2">
                  <RadioGroupItem value={opt} id={`willing-${opt}`} />
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea value={fearSentence} onChange={(e) => setFearSentence(e.target.value)} rows={2} />
          </div>
          <div>
            <Label className="font-medium">Is this fear emotional… or factual?</Label>
            <Textarea value={fearOrFactual} onChange={(e) => setFearOrFactual(e.target.value)} placeholder="Reflect on whether this fear is based on emotion or evidence..." rows={2} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <RadioGroup value={unityCheck} onValueChange={setUnityCheck} className="space-y-2">
              {["Fully aligned", "Some disagreement", "Deep division"].map((opt) => (
                <div key={opt} className="flex items-center space-x-2">
                  <RadioGroupItem value={opt} id={`unity-${opt}`} />
          ...
          {(unityCheck === "Some disagreement" || unityCheck === "Deep division") && (
            <div>
              ...
              <Textarea value={unityResolve} onChange={(e) => setUnityResolve(e.target.value)} rows={2} />
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Textarea value={communicationScript} onChange={(e) => setCommunicationScript(e.target.value)} placeholder="Write your script..." rows={4} />
          </div>
      ...
      <Card className="border-primary/30 bg-gradient-to-br from-primary/5 to-primary/10">
        ...
        <CardContent className="space-y-4">
          ...
          <Textarea value={selfReflection} onChange={(e) => setSelfReflection(e.target.value)} placeholder="Reflect honestly..." rows={3} />
        </CardContent>
      </Card>
      ...
      <div className="flex gap-3 print:hidden">
        ...
        <Button variant="outline" onClick={handleReset}>Reset Worksheet</Button>
      </div>
    </div>
  );
}
