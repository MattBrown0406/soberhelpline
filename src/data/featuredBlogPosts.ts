import earlyInterventionFamilyChangesFirstImg from "@/assets/blog-early-intervention-family-changes-first.png";
import earlyInterventionFamilySkillImg from "@/assets/blog-early-intervention-family-skill.png";
import earlyInterventionStabilizingFamilyImg from "@/assets/blog-early-intervention-stabilizing-family.png";
import addictionConfusionFamiliesImg from "@/assets/blog-addiction-confusion-families.png";
import depressionAddictionFamiliesImg from "@/assets/blog-depression-addiction-families.png";
import whatToExpectInterventionImg from "@/assets/blog-what-to-expect-during-intervention.jpg";

export const featuredBlogPosts = [
  {
    id: 109,
    slug: "what-to-expect-during-an-intervention",
    title: "What to Expect During an Intervention: A Real Guide for Families",
    excerpt: "Most of what families fear about intervention day simply doesn't happen. Here is a clear, step-by-step look at what really takes place — from preparation to the conversation to what comes next.",
    author: "Matt Brown",
    date: "2026-04-24",
    category: "Intervention",
    image: whatToExpectInterventionImg,
  },
  {
    id: 74,
    slug: "depression-and-addiction-family-confusion",
    title: "Depression and Addiction: Why Families Struggle to Tell What's Causing What, and Why It Matters",
    excerpt: "Families often get stuck trying to decide whether depression caused the addiction or addiction caused the depression. The more useful question is what kind of help the whole picture requires.",
    author: "Matt Brown",
    date: "2026-01-16",
    category: "Mental Health",
    image: depressionAddictionFamiliesImg,
  },
  {
    id: 73,
    slug: "understanding-addiction-confusion-families",
    title: "Why Addiction Feels So Confusing to Families, and How to Start Making Sense of It",
    excerpt: "Addiction creates emotional chaos, mixed signals, and constant second-guessing. This guide helps families understand why it feels so disorienting and where clarity starts.",
    author: "Matt Brown",
    date: "2026-01-14",
    category: "Family Education",
    image: addictionConfusionFamiliesImg,
  },
  {
    id: 66,
    slug: "early-intervention-reducing-family-damage",
    title: "Early Intervention Isn't About Readiness, It's About Reducing Family Damage",
    excerpt: "Families do not need a crisis or a commitment to treatment before they deserve support. Early intervention helps reduce chaos, guilt, and long-term damage now.",
    author: "Matt Brown",
    date: "2026-01-12",
    category: "Early Intervention",
    image: earlyInterventionFamilyChangesFirstImg,
  },
  {
    id: 67,
    slug: "early-intervention-family-changes-first",
    title: "Why Early Intervention Works Best When the Family Changes First",
    excerpt: "The earliest leverage point is usually not the person using substances. It is the family system around them. Here's why that matters.",
    author: "Matt Brown",
    date: "2026-01-11",
    category: "Early Intervention",
    image: earlyInterventionFamilySkillImg,
  },
  {
    id: 69,
    slug: "early-intervention-stabilizing-family-first",
    title: "Early Intervention Isn't About Forcing Treatment, It's About Stabilizing the Family First",
    excerpt: "When panic drives every decision, families lose clarity. Stabilization creates the conditions for better choices, better boundaries, and better outcomes.",
    author: "Matt Brown",
    date: "2026-01-09",
    category: "Early Intervention",
    image: earlyInterventionStabilizingFamilyImg,
  },
];
