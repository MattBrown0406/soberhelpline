...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Only require auth for hosts (role=1). Guests (role=0) can join freely.
    if (Number(role) !== 0) {
    ...
    const signature = await generateSignature(sdkKey, sdkSecret, String(meetingNumber), Number(role));
