...
-- This makes the view respect RLS policies of underlying tables (provider_clicks requires admin role)
...
SELECT 
    ...
    count(CASE WHEN pc.clicked_at > (now() - '7 days'::interval) THEN 1 ELSE NULL END) AS clicks_last_7_days,
    count(CASE WHEN pc.clicked_at > (now() - '30 days'::interval) THEN 1 ELSE NULL END) AS clicks_last_30_days,
