-- Fix: Use security_invoker = true (default) and add RLS policy for public access to approved providers
-- This is the recommended pattern instead of security definer views

-- First, add RLS policy to allow public SELECT on approved providers only
...
-- Now recreate the view with security_invoker = true (uses querying user's RLS)
